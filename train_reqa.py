...
def test(model, question_embeddings, candidate_embeddings, test_ground_truth, used_dimension, seed, fold_idx):
    if used_dimension != -1:
...
def run(model, train_data_loader, dev_data_loader, test_question_data_loader, test_candidate_data_loader, args, seed, results):
    ...
    if args.do_test == "True":
        ...
        test_mrr, test_p1, test_r5, test_r10 = test(model,
                                                    copy.deepcopy(question_embeddings), 
        ...
        if args.whitening == "True":
            ...
            whitening = obtain_whitening_params(model, [train_data_loader], args)
            if not os.path.exists(args.save_model_path):
                os.makedirs(args.save_model_path)
            save_dict = {
                "model_state_dict": model.state_dict(),
            ...
            torch.save(save_dict, model_name)
            whitened_question_embeddings, whitened_candidate_embeddings = whiten_sentence_embeddings(question_embeddings, 
                                                                                    candidate_embeddings, 
                                                                                    whitening, args)


            test_mrr, test_p1, test_r5, test_r10 = test(model,
                                                        whitened_question_embeddings, 
                                                        whitened_candidate_embeddings, 
                                                        test_ground_truth, 
                                                        -1, 
                                                        seed,
                                                        args.fold_idx)
...
def prepare_model(args):
    ...
    if args.load_model_path != "":
        pretrained_model_dict = torch.load(args.load_model_path.format(fold_idx=args.fold_idx))["model_state_dict"]
...
def main():
    ...
    for seed in args.seeds:
        ...
        for fold_idx, (train_ids, dev_ids) in enumerate(kf):
            ...
            # loading dataset
            train_data_loader, dev_data_loader, test_question_data_loader, test_candidate_data_loader = prepare_dataloaders(args)

            # preparing model
            model = prepare_model(args)

            # running
            run(model, train_data_loader, dev_data_loader, test_question_data_loader, test_candidate_data_loader, args, seed, results)

        if args.do_test == "True":
            # logger.info(f"Current random seed: {seed}")
            results[seed]["normal"]["mrr"] = np.mean(results[seed]["normal"]["mrr"])
            results[seed]["normal"]["p1"] = np.mean(results[seed]["normal"]["p1"])
            results[seed]["normal"]["r5"] = np.mean(results[seed]["normal"]["r5"])
            results[seed]["normal"]["r10"] = np.mean(results[seed]["normal"]["r10"])
            if args.whitening == "True":
                results[seed]["whiten"]["mrr"] = np.mean(results[seed]["whiten"]["mrr"])
                results[seed]["whiten"]["p1"] = np.mean(results[seed]["whiten"]["p1"])
                results[seed]["whiten"]["r5"] = np.mean(results[seed]["whiten"]["r5"])
                results[seed]["whiten"]["r10"] = np.mean(results[seed]["whiten"]["r10"])
    ...
    results["overall"]["normal"]["mrr"]["mean"] = np.mean([results[seed]["normal"]["mrr"] for seed in args.seeds])
    results["overall"]["normal"]["mrr"]["std"] = np.std([results[seed]["normal"]["mrr"] for seed in args.seeds])
    results["overall"]["normal"]["p1"]["mean"] = np.mean([results[seed]["normal"]["p1"] for seed in args.seeds])
    results["overall"]["normal"]["p1"]["std"] = np.std([results[seed]["normal"]["p1"] for seed in args.seeds])
    results["overall"]["normal"]["r5"]["mean"] = np.mean([results[seed]["normal"]["r5"] for seed in args.seeds])
    results["overall"]["normal"]["r5"]["std"] = np.std([results[seed]["normal"]["r5"] for seed in args.seeds])
    results["overall"]["normal"]["r10"]["mean"] = np.mean([results[seed]["normal"]["r10"] for seed in args.seeds])
    results["overall"]["normal"]["r10"]["std"] = np.std([results[seed]["normal"]["r10"] for seed in args.seeds])

    logger.info(f'[Overall][Normal] MRR ={results["overall"]["normal"]["mrr"]["mean"]: 3.2f} %, '\
                                               f'P@1 ={results["overall"]["normal"]["p1"]["mean"]: 3.2f} %, '\
                                               f'R@5 ={results["overall"]["normal"]["r5"]["mean"]: 3.2f} %, '\
                                               f'R@10 ={results["overall"]["normal"]["r10"]["mean"]: 3.2f} %.')
    if args.whitening == "True":
        results["overall"]["whiten"] = defaultdict(dict)
        results["overall"]["whiten"]["mrr"]["mean"] = np.mean([results[seed]["whiten"]["mrr"] for seed in args.seeds])
        results["overall"]["whiten"]["mrr"]["std"] = np.std([results[seed]["whiten"]["mrr"] for seed in args.seeds])
        results["overall"]["whiten"]["p1"]["mean"] = np.mean([results[seed]["whiten"]["p1"] for seed in args.seeds])
        results["overall"]["whiten"]["p1"]["std"] = np.std([results[seed]["whiten"]["p1"] for seed in args.seeds])
        results["overall"]["whiten"]["r5"]["mean"] = np.mean([results[seed]["whiten"]["r5"] for seed in args.seeds])
        results["overall"]["whiten"]["r5"]["std"] = np.std([results[seed]["whiten"]["r5"] for seed in args.seeds])
        results["overall"]["whiten"]["r10"]["mean"] = np.mean([results[seed]["whiten"]["r10"] for seed in args.seeds])
        results["overall"]["whiten"]["r10"]["std"] = np.std([results[seed]["whiten"]["r10"] for seed in args.seeds])

        logger.info(f'[Overall][Whiten] MRR ={results["overall"]["whiten"]["mrr"]["mean"]: 3.2f} %, '\
                                                    f'P@1 ={results["overall"]["whiten"]["p1"]["mean"]: 3.2f} %, '\
                                                    f'R@5 ={results["overall"]["whiten"]["r5"]["mean"]: 3.2f} %, '\
                                                    f'R@10 ={results["overall"]["whiten"]["r10"]["mean"]: 3.2f} %.')
    

    if args.save_results == "True":
        if not os.path.exists(args.save_model_path):
            os.makedirs(args.save_model_path)
        with open(args.save_model_path + "/results.json", 'w', encoding='utf-8') as f:
