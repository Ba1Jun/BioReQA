...
def prepare_tokenizer(args):
    if args.encoder_type == 'biobert':
        from transformers import BertTokenizer
        tokenizer = BertTokenizer.from_pretrained(args.plm_path, do_lower_case=False)
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def padding(self, input_ids, max_len):
        ...
        attention_mask = np.array([[1] * len(item) + [self.pad_token_id] * (max_len-len(item)) for item in _input_ids], dtype=np.int)
...
class NliDataset(BaseDataset):
    def __init__(self, args, split='train'):
        super(NliDataset, self).__init__(args)
        self.split = split
        self.data_file = 'data/NLI/AllNLI.tsv'
        self.process()
    ...
    def process(self):
        ...
        dataset_name = 'nli'
        cached_dataset_file = os.path.join(cached_dir, f"{dataset_name}_{self.split}_{plm_name}")
        ...
        if os.path.exists(cached_dataset_file) and not self.args.overwrite_cache:
            ...
            self.premise_input_ids = data_dict["premises"]
            self.hypothesis_input_ids = data_dict["hypotheses"]
        else:
            logging.info("Creating instances from dataset file at %s", self.data_file)
            raw_data = pd.read_csv(self.data_file, sep='\t', error_bad_lines=False)
            entailment_data = raw_data[raw_data['label']=='entailment']
            if self.split == 'train':
                used_data = entailment_data[entailment_data['split']=='train']
            elif self.split == 'dev':
                used_data = entailment_data[entailment_data['split'].isin(['dev', 'test'])]
            premises = used_data['sentence1'].tolist()
            hypotheses = used_data['sentence2'].tolist()
            # tokenize
            premise_tokens, self.premise_input_ids = self.tokenize(premises, 'premise   ')
            hypothesis_tokens, self.hypothesis_input_ids = self.tokenize(hypotheses, 'hypothesis')
            
            logging.info(f"premise: {premises[0]}")
            logging.info(f'premise tokens: {premise_tokens[0]}')
            logging.info(f'premise input ids: {self.premise_input_ids[0]}')
            logging.info('')
            logging.info(f"hypothesis: {hypotheses[0]}")
            logging.info(f'hypothesis tokens: {hypothesis_tokens[0]}')
            logging.info(f'hypothesis input ids: {self.hypothesis_input_ids[0]}')
            logging.info('')
            # save data
            saved_data = {
                'premises': self.premise_input_ids,
                'hypotheses': self.hypothesis_input_ids
            }
            logging.info("Saving processed dataset to %s", cached_dataset_file)
            torch.save(saved_data, cached_dataset_file)
        
    def __len__(self):
        return len(self.premise_input_ids)
    
    def __getitem__(self, idx):
        return (self.args, 
                self.premise_input_ids[idx],
                self.hypothesis_input_ids[idx])
    
    def collate_fn(self, raw_batch):
        args = raw_batch[-1][0]
        batch = dict()
        _, premise_input_ids, hypothesis_input_ids = list(zip(*raw_batch))
        batch['src_input_ids'], batch['src_attention_mask'] = self.padding(premise_input_ids, self.args.max_premise_len)
        batch['tgt_input_ids'], batch['tgt_attention_mask'] = self.padding(hypothesis_input_ids, self.args.max_hypothesis_len)
...
class BioASQDataset(BaseDataset):
    def __init__(self, args, split='train'):
        super(BioASQDataset, self).__init__(args)
        self.split = split
    ...
    def process(self):
        # Load data features from cache or datas et file
        cached_dir = f"./cached_data/{self.version}"
        if not os.path.exists(cached_dir):
            os.makedirs(cached_dir)
        plm_name = [s for s in self.args.plm_path.split('/') if s !=''][-1]
        ...
        if os.path.exists(cached_dataset_file) and not self.args.overwrite_cache:
            ...
            with open(self.data_folder + f'/{"cv_data" if self.split in ["train", "dev"] else "test_data"}.json', 'r', encoding='utf-8') as f:
        ...
        if self.split != 'test':
            ...
            self.question_input_ids = [input_ids for qid, input_ids in zip(self.question_ids, self.question_input_ids) if qid in data_ids]
            self.answer_input_ids = [input_ids for qid, input_ids in zip(self.question_ids, self.answer_input_ids) if qid in data_ids]
            self.question_ids = [qid for qid in self.question_ids if qid in data_ids]
    ...
    def collate_fn(self, raw_batch):
        ...
        batch = dict()
        if self.split == 'test':
            _, question_input_ids, answer_input_ids = list(zip(*raw_batch))
            batch['src_input_ids'], batch['src_attention_mask'] = self.padding(question_input_ids, -1)
            batch['tgt_input_ids'], batch['tgt_attention_mask'] = self.padding(answer_input_ids, -1)
        else:
            _, question_input_ids, answer_input_ids, question_ids = list(zip(*raw_batch))
            batch['src_input_ids'], batch['src_attention_mask'] = self.padding(question_input_ids, self.args.max_question_len)
            batch['tgt_input_ids'], batch['tgt_attention_mask'] = self.padding(answer_input_ids, self.args.max_answer_len)
